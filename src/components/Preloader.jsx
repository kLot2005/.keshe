    ...
    useEffect(() => {
        ...
        class Particle {
            constructor(x, y, isTextParticle, targetChar = '', charIndex = -1) {
                ...
                this.size = 28; // Grid character size (fixed size)
                this.alpha = 1.0;
        ...
        const render = () => {
            ...
            // Transition checks
            if (currentStage === 1 && (loaded || elapsed > 4000) && elapsed >= minStage1Time) {
                currentStage = 2; // Transition to reveal/stabilization
                stage2StartTime = Date.now();
            } 
            else if (currentStage === 2 && (Date.now() - stage2StartTime) >= stage2Duration) {
